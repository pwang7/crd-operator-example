...
def kill_pods(labels):
    ...
    for label in labels:
        ...
        log.info("kill pod url={}".format(url))
        r = requests.get(url)
        # Make the request to the endpoint to retreive the Pods
        response = r.json()
        log.info("pod label selector result={}".format(response))
...
def getPodLabels(configmap):
    url = "{}/apis/magalix.com/v1/namespaces/{}/configmonitors".format(
        base_url, namespace)
    log.info("get pod labels url={}".format(url))
    r = requests.get(url)
    # Issue the HTTP request to the appropriate endpoint
    response = r.json()
    log.info("get pod label result={}".format(response))
